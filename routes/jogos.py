...
def editar_jogo_completo(
    ...
    cursor = conn.cursor()
    try:
        query = "UPDATE jogos SET titulo = %s, plataforma = %s, preco_aluguel = %s, preco_aluguel_14 = %s, descricao = %s, url_imagem = %s, tempo_jogo = %s, nota = %s, data_lancamento = %s WHERE id = %s"
        cursor.execute(
            query,
            (
                dados.titulo,
