...
def listar_jogos(usuario_id: int = 0):
    ...
    try:
        ...
        if usuario_id > 0:
            ...
            resultado_rank = cursor.fetchone()
            if resultado_rank:
                rank_cliente = min(resultado_rank["rank"], 20)

        fator_desconto = 1.0 - (rank_cliente / 100.0)
        ...
        query = """
            SELECT j.id, j.titulo, j.plataforma, j.descricao, j.url_imagem, j.tempo_jogo, j.nota, CAST(j.data_lancamento AS VARCHAR) as data_lancamento,
                ...
                -- Fila e devolução são SEPARADAS por vaga: a fila da primária não
                -- atrasa a secundária (e vice-versa). Misturar as duas fazia a vitrine
                -- somar a volta de uma vaga com a fila da outra.
                (SELECT COUNT(*) FROM fila_espera WHERE jogo_id = j.id AND status = 'AGUARDANDO' AND tipo_slot = 'PRIMARIA') AS fila_primaria,
                (SELECT COUNT(*) FROM fila_espera WHERE jogo_id = j.id AND status = 'AGUARDANDO' AND tipo_slot = 'SECUNDARIA') AS fila_secundaria,
                (SELECT COALESCE(SUM(dias_aluguel), 0) FROM fila_espera WHERE jogo_id = j.id AND status = 'AGUARDANDO' AND tipo_slot = 'PRIMARIA') AS fila_dias_primaria,
                (SELECT COALESCE(SUM(dias_aluguel), 0) FROM fila_espera WHERE jogo_id = j.id AND status = 'AGUARDANDO' AND tipo_slot = 'SECUNDARIA') AS fila_dias_secundaria,
                (SELECT MIN(l.data_fim) FROM locacoes l JOIN contas_psn c ON l.conta_psn_id = c.id WHERE c.jogo_id = j.id AND l.status = 'ATIVA' AND l.tipo_slot = 'PRIMARIA') AS proxima_devolucao_primaria,
                (SELECT MIN(l.data_fim) FROM locacoes l JOIN contas_psn c ON l.conta_psn_id = c.id WHERE c.jogo_id = j.id AND l.status = 'ATIVA' AND l.tipo_slot = 'SECUNDARIA') AS proxima_devolucao_secundaria,
        ...
        query_segura = """
            SELECT j.id, j.titulo, j.plataforma, j.preco_aluguel, j.preco_aluguel_14,
                ...
                (SELECT COUNT(*) FROM fila_espera WHERE jogo_id = j.id AND status = 'AGUARDANDO' AND tipo_slot = 'PRIMARIA') AS fila_primaria,
                (SELECT COUNT(*) FROM fila_espera WHERE jogo_id = j.id AND status = 'AGUARDANDO' AND tipo_slot = 'SECUNDARIA') AS fila_secundaria,
                0 AS fila_dias_primaria,
                0 AS fila_dias_secundaria,
                (SELECT MIN(l.data_fim) FROM locacoes l JOIN contas_psn c ON l.conta_psn_id = c.id WHERE c.jogo_id = j.id AND l.status = 'ATIVA' AND l.tipo_slot = 'PRIMARIA') AS proxima_devolucao_primaria,
                (SELECT MIN(l.data_fim) FROM locacoes l JOIN contas_psn c ON l.conta_psn_id = c.id WHERE c.jogo_id = j.id AND l.status = 'ATIVA' AND l.tipo_slot = 'SECUNDARIA') AS proxima_devolucao_secundaria,
                (SELECT COUNT(*) FROM locacoes l JOIN contas_psn c ON l.conta_psn_id = c.id WHERE c.jogo_id = j.id) AS popularidade,
                (SELECT COALESCE(SUM(CASE WHEN t.tipo = 'SAIDA' THEN t.valor WHEN t.tipo = 'ENTRADA' THEN -t.valor ELSE 0 END), 0) FROM transacoes t WHERE t.utilizador_id != 1 AND t.descricao ILIKE '%%' || j.titulo || '%%') AS faturamento_total,
