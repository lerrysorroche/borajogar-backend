    ...
    for r in reservas:
        ...
        if eh_pre_venda:
            ...
            cursor.execute(
                """
                SELECT COALESCE(SUM(dias_aluguel), 0) as dias_frente FROM fila_espera 
                WHERE jogo_id = %s AND status = 'AGUARDANDO' AND data_solicitacao < %s
            """,
...
def gerar_codigo_2fa(locacao_id: int, usuario_id: int):
    ...
    if not resultado or not resultado["mfa_secret"]:
        raise HTTPException(
            status_code=404,
            detail="A autenticação não está configurada para esta conta ou a locação expirou.",
        )
    ...
    try:
        ...
        cursor.execute(
            ...
            (
                ...
                preco,
                f"Reserva na Fila ({reserva.dias_aluguel}d): {titulo}",
            ),
        )
            ...
            if meus_alugueis_qtd > 0:
                ...
                for b in bumped:
                    ...
                    cursor.execute(
                        ...
                        (b["utilizador_id"], b["id"], titulo, msg),
                    )
...
def realizar_locacao(locacao: NovaLocacao):
    ...
    try:
        ...
        cursor.execute(
            query_recibo, (locacao.utilizador_id, conta["id"], locacao.dias_aluguel)
        )
        ...
        cursor.execute(
            ...
            (
                ...
                preco,
                f"Aluguel ({locacao.dias_aluguel}d): {titulo}",
            ),
        )
        conn.commit()
        return {
            "mensagem": "Aluguel realizado! Valor descontado.",
            "pedido_id": recibo["id"],
...
def liberar_conta_manutencao(
    ...
    try:
        ...
        hoje_str = datetime.now().strftime("%Y-%m-%d")
        eh_pre_venda = data_lanc and str(data_lanc) >= hoje_str
        ...
        cursor.execute(query_fila, (jogo_id,))
        ...
        if proximo_da_fila:
            ...
            cursor.execute(
                ...
                (proximo_da_fila["utilizador_id"], dados.conta_psn_id, dias_comprados),
            )
            cursor.execute(
...
def admin_cancelar_reserva(reserva_id: int, admin_data=Depends(verificar_admin)):
    ...
    try:
        ...
        if not res:
            raise HTTPException(
                status_code=400, detail="Reserva não encontrada ou já processada."
            )
        ...
        if reembolso > 0:
            ...
            cursor.execute(
                "INSERT INTO transacoes (utilizador_id, tipo, valor, descricao) VALUES (%s, 'ENTRADA', %s, %s)",
                (usr_id, reembolso, f"💸 Estorno (Admin): {titulo}"),
            )
        ...
        conn.commit()
        return {
            "mensagem": f"Reserva de {titulo} cancelada e valor estornado ao cliente."
        }
    except Exception as e:
        conn.rollback()
        raise HTTPException(status_code=400, detail=str(e))
            ...
            for conta in contas_disponiveis:
                ...
                if eh_pre_venda:
                    ...
                    query_proximo = """
                        ...
                        ORDER BY data_solicitacao ASC LIMIT 1
                    """
                cursor.execute(query_proximo, (jogo_id,))
...
def forcar_filas(admin_data=Depends(verificar_admin)):
    ...
    return {
        "mensagem": "O motor de filas rodou com sucesso! Verifique a aba de Locações Ativas."
    }
